...
// BenchmarkSettleThroughput measures end-to-end accept→settle throughput:
// parallel emitters through a manual source, a four-way fan-out (four sink
// workers settling concurrently), then a full WaitSettled barrier inside the
...
// out-of-lock persistence refactor targets: while one settle flushes, the
// tracker must stay available to arrivals and other settles).
func BenchmarkSettleThroughput(b *testing.B) {
	const benchYAML = `
...
`
	for _, tc := range []struct {
		...
		b.Run(tc.name, func(b *testing.B) {
			...
			if got := eng.Metrics.SettledCount.Load(); got != int64(b.N) {
				b.Fatalf("settled %d, want %d", got, b.N)
			}
			b.StopTimer()
		})
	}
