...
每个插件注册时同时注册 schema（含字段类型、默认值、约束、描述文本）。v1.17 起内置插件的 schema 从**插件自己的 config 结构体**生成（`registry.RegisterSourceT/RegisterSinkT/RegisterCodecT`，`schema` 结构体标签声明约束与默认值）——结构体是单一事实来源：schema 文档、严格校验、运行时默认值注入三者的数据同源，不再有"手写 schema 与工厂代码各自维护一份"的漂移面。外部 gRPC 插件仍以 manifest 声明 schema 字符串（协议不变）：
