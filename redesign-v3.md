...
5. **语义 lint**（超出"对错"的告警，`--strict` 时升级为错误）：
   - `run.mode: batch` 且无源声明 finite 取尽 → 运行将挂至取消（`batch_no_finite_source`，v1.24）；作业管道的 file 源未设 `on_eof: stop` → 作业永不完成（`job_file_source_no_eof`，v1.24）；
...
| 层 | 内容 | 质量标准 |
|----|------|----------|
| 内置 source | `kafka`、`http_server`、`cron`、`file`（tail，`on_eof: stop` 后为有限源，v1.24；`capabilities: [pull, finite]`）；`sql`（mysql/postgres/sqlite——sqlite 方言服务于开发/示例/CI，拉取型） | 生产级：重试、背压、offset/checkpoint/水位、指标、故障注入测试 |
| 内置 sink | `kafka`、`http`、`file`（滚动）、`drop`；`debug`（stderr 调试边，v1.22） | 生产级：批量、幂等指引、死信语义（`debug` 为调试便利边，不在此要求内） |
...
**拉取型源能力位**：插件 schema 声明 `capabilities: [pull]`（可被作业调度：分页、水位、取尽信号）。`run.mode: job` 引用非 pull 源 = verify 错误。调度、补偿、重叠、历史全部由引擎在 pipeline 层一次实现（§5.8），源插件只回答"按参数取下一批"。**有限性与 pull 正交（v1.24）**：pull 能力位的本质是"分页、水位、取尽信号"——声明了 `pull` 但未实现 `PullSource` 的源（如 `file` + `on_eof: stop`）凭 `Run` 的取尽返回同样可被作业调度（作业运行按接口断言分派：有 `Pull` 调 Pull，否则 Run）；连续管线引用 pull 源给 lint 提示，batch 管线（`run.mode: batch`，连续形状 + 完成策略：全源取尽且全部 commit 后进程退出/状态置 completed）则视 pull 源为一等公民。
...
```
eventboat run        --config / --config-dir          运行（含热重载信号）；run.mode: batch 时读到全源取尽 + 全部 commit 即自动退出（v1.24，退出码对齐 trigger：success=0/partial|failed=1）
eventboat verify     --config [--strict] [--json]     静态验证（关卡一）
eventboat test       <file-or-dir> [--json]           合约测试（关卡二）
eventboat explain    --config [--message f.json]      路径推演（关卡三）
eventboat replay     --dlq / --spool / --job [...]    死信/窗口/作业回放（关卡三）
eventboat trigger    --config <job.yaml> [--parameters json]   手动触发作业（§5.8）
eventboat jobs       list | show <run-id>             作业历史
eventboat repl       [--script f.star] [--cel 'expr'] 脚本/表达式 REPL（§4.4）
eventboat lsp        （stdio，无旗标）                  语言服务器（§4.11）
eventboat plugin     catalog | schema <name>          插件清单与 Schema（§6.5）
eventboat mcp        [--stdio | --http]               启动 MCP Server（关卡四）
eventboat help       [<verb>]                         帮助三级：无参 / -h / help <verb>
```

（2026-09-04：分发层迁移至自有零依赖库 [lynx-go/commands](https://github.com/lynx-go/commands) v0.2.0——`--json` 为全局旗标（动词前后皆可），usage 类错误统一退出码 2 并附 `usage:` 提示行，帮助输出由库渲染并以 golden 快照钉住；上表按实现对齐：test 为位置参数、trigger 经 `--config`、plugin 子命令为 catalog/schema，补 lsp 与 help 行。）
...
每个插件注册时同时注册 schema（含字段类型、默认值、约束、描述文本）。v1.17 起内置插件的 schema 从**插件自己的 config 结构体**生成（`registry.RegisterSourceT/RegisterTransformT/RegisterSinkT/RegisterCodecT`，`schema` 结构体标签声明约束与默认值）——结构体是单一事实来源：schema 文档、严格校验、运行时默认值注入三者的数据同源，不再有"手写 schema 与工厂代码各自维护一份"的漂移面。外部 gRPC 插件仍以 manifest 声明 schema 字符串（协议不变）：

- Loader 校验两层：node 层框架字段按白名单枚举（未知框架字段=错误），插件块按插件 schema 严格校验（未知字段=错误）——**两套命名空间物理隔离，互不冲突**。白名单、顶层键、边属性、保留插件名与节点级默认值同源于叶子包 `internal/framework`（config/registry/LSP 读同一份，v1.25）；管道级默认值（decoder/encoder `json`、delivery `retries: 3` 等）在加载期物化进 typed config，下游不再二次补默认值；
...
1. **职责切割**：调度、错过补偿、重叠策略、作业历史、参数绑定——全部由引擎在 pipeline 层实现一次；源插件只实现"按参数取下一批"（`capabilities: [pull]`：分页、水位、取尽信号）。verify 检查 `run.mode: job` 的所有源具备 pull 能力。无 `Pull` 方法但 `Run` 自终止的有限源（file + `on_eof: stop`）同样可被调度（v1.24：取尽信号由 Run 返回值承载）；file 源不参与 `cursor` 参数绑定（整文件语义，字节偏移即水位）。
2. **作业生命周期**：`pending → running（源分页拉取 + DAG 执行，在途消息 commit 完毕后落终态）→ success | partial（有死信）| failed | canceled`；每次运行产生一条**作业历史记录**（run-id、parameters、起止、行数/投递/死信计数），存同一 SQLite store，按 `run.retention` 清理。终态判定由引擎 `Outcome` 唯一给出（v1.25）；`committing` 中间态已删除——它从来没有写入者。
...
| `codecs` | 可选 | 命名 codec 复用（如带 schema 的 avro），`decoder`/`encoder` 按名引用 | v2 同名延续；M4 落地（v1.14）：`name: {type: <注册 codec 名>, ...该 codec 的配置}`，声明名与注册 codec 名是两个名字空间、遮蔽 = verify 错误；配置按 codec 的 JSON Schema 严格校验（与插件同诊断格式） |
| `dlq` | 可选 | 死信策略：`retention`（时长，如 `30d`）——超期死信由引擎在 checkpoint 保留窗口内分批清扫（落地见 v1.21） | 约定俗成缩写，保留；**缺省 0 = 永久保留，无非零默认（opt-in）**：死信是操作员要 `replay` 的数据，自动删除等于静默销毁，显式配置后删除的行不可再回灌；死信存储本身是引擎机制不是 sink |
...
- **checkpoint**：committed 消息的 spool 位点持久化 + source 位点提交（Kafka: consumer offset；file: 文件 offset；http_server: 无位可提交，spool 即真相；sql: 水位列，见 §5.8）。source 位点由每源一个异步提交器持久化（合并最大前沿，v1.25）：滞后只扩大崩溃后的重复投递窗口，绝不丢数据。
- **投递语义**：sink 失败按边 `delivery`（retries×backoff、timeout）重试；耗尽 → **死信库**（存 spool 同一存储，含完整原始消息 + 错误 + Starlark backtrace/CEL 错误 + **失败类别 `class`**——在生产处定型，绝不回读原因文本，v1.25）；死信写入成功 = 该分支终态（commit）。
...
spool 表设计要点：append-only 消息表 + checkpoint 表 + 死信表（索引：时间、node、原因、route）+ **作业历史表**（run-id、parameters、状态、计数）；`replay` 命令即 SQL 查询 + 重新注入；`--ephemeral` 标志可关闭持久化（本地开发/测试）。落地布局收敛为 `<data-dir>/stores/<sanitized pipeline>.db`（每管道一库、每进程一个缓存句柄，`store.Owner`；旧布局退役不迁移，v1.25），持久化面按关注点拆为 spool/死信/作业历史三个 facet；运行中的引擎在库文件旁持有独占 OS 文件锁 `<store>.lock`——**单写者租约**：跨进程保证每管道一个写者，崩溃即释放（无 TTL 心跳），一次性写动词拿不到锁即拒绝并指向 admin/MCP（v1.25）。dagu 从 JSON 文件迁到 SQLite 的历史与 [SQLite 做 durable 执行引擎](https://www.morling.dev/blog/building-durable-execution-engine-with-sqlite/)的实践是这个选型的旁证。
...
M3 落地要点（v1.12）：WASM guest = wasip1 **reactor**（`_initialize`），标准 Go 工具链 `go build -buildmode=c-shared` 即可构建；消息 wire = payload JSON 字节进出、meta 透传、错误沿 script 同路 delivery→死信。gRPC 插件 = stdout 单行 JSON 握手（协议版本 + 名称 + 版本 + 能力 + 监听地址 + 一次性令牌）+ 静态 **manifest** 文件（verify 据此校验配置，**不 spawn 进程**）+ 运行时握手与 manifest 交叉核对；崩溃 = 快速失败（自动重启列为后续）。

WASM 资源模型（v1.13，M3 审核 J2 裁决修订）：wazero 无指令计量，预算 = 每次调用 wall-clock + 内存页上限；**`timeout_ms` 缺省 0（快速模式，无击杀开关），保护为显式开启**——ctx 击杀机制实测约 5× 开销，默认带击杀会使本档慢于 Starlark、与其存在理由（§4.5）矛盾；失控 guest 属可用性风险（卡一个 worker、背压停摆、不丢数据、重启即愈）而非正确性风险。未设置时 verify 告警（`wasm_no_kill_switch`，`--strict` 升级为错误）+ 慢调用看门狗日志（阈值默认 5s 可配，每次调用至多告警一次，零干涉不击杀）。
...
- **HA**：单活实例 per pipeline（spool 在本地盘）；故障 = 重调度 + 从 source 位点/spool 恢复（at-least-once 兜底）；作业管道靠补偿窗口（`catchup_window`）+ 水位续传。单活由运行中引擎对存储文件的独占 OS 文件锁保证（v1.25，见 §6.3）：跨进程只有一个写者，锁随进程死亡释放。多活分片（按 `order_key` 分区）列 P2 非目标——诚实：v3 不承诺水平扩展，承诺单机可靠与快速故障恢复。
...
internal/
  ...
  framework/         框架词汇唯一来源：节点字段白名单、顶层键、边属性、保留插件名、节点级默认值（config/registry/LSP 同源，v1.25）
  ir/                静态 IR：DAG(nodes,edges)、编译产物、lint；explain 的 ForExplain 干跑实例
  explain/           消息级推演与拓扑渲染（CEL/Starlark 干跑；v1.25 渲染已解析语义）
  verify/            唯一的 load → build → judge 组合（File/Bytes + 两段式；CLI/MCP/Admin/LSP/jobs/testrun/explain 共用，v1.25）
  dlq/               死信策略面：过滤编译（带 constants）、选择顺序、带 codec 的重放请求（v1.25）
  fsname/            文件名规则（Sanitize + Windows 保留设备名；config 与 store 共用，v1.25）
...
- 二进制名 `eventboat`；CLI：`eventboat verify / test / run / deploy`；指标前缀 `eventboat_*`；`apiVersion: eventboat/v1`；skills.sh 与 MCP 发布名同源。
