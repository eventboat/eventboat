...
**Goal:** Make riverpod operable by external AI agents — Cursor Skill (done), machine-readable CLI, optional MCP server — **before** in-pipeline LLM transforms.

**Architecture:** Skill documents CLI + Admin API workflow; new CLI flags emit JSON; MCP wraps the same operations as thin tools.

**Tech Stack:** Go 1.22+, existing `cmd/riverpod`, `mcp-go` or stdlib JSON-RPC over stdio (evaluate at Task 4).
...
| Deliverable | Path |
|-------------|------|
| Project Skill | `skills/riverpod/SKILL.md` |
| Extended reference | `skills/riverpod/reference.md` |
| Skills catalog | `skills/README.md` |
| Design doc | `docs/ai-agent.md` § Phase 0 |

### Manual acceptance

- [ ] Open Cursor in riverpod repo; ask Agent to create a cron→map→drop pipeline
- [ ] Agent reads Skill, runs `riverpod validate`, fixes errors without reading Go source
- [ ] Agent runs `riverpod test` or uses `_examples/01-linear-etl.yaml` as reference
...
### Task 1: `riverpod plugins list`

**Files:** `cmd/riverpod/main.go`, `internal/registry/registry.go` (read-only list API)

- [ ] **1.1** Add `ListSources()`, `ListTransforms()`, `ListSinks()`, `ListCodecs()` on Registry
- [ ] **1.2** Add subcommand `riverpod plugins list [--format text|json]`
- [ ] **1.3** JSON shape: `{ "sources": ["kafka",...], "transforms": [...], "sinks": [...], "codecs": [...] }`
- [ ] **1.4** Test + update Skill reference

### Task 2: `riverpod validate --format json`

**Files:** `cmd/riverpod/main.go`, `internal/topology/` (wrap errors)
...
### Task 3: `riverpod doc` (optional, stretch)

**Files:** `cmd/riverpod/doc.go`, `internal/topology/`

- [ ] **3.1** `riverpod doc --config FILE --format dot` → Graphviz DOT of stages/edges
...
- [ ] **4.2** `riverpod mcp` subcommand starts server on stdio
...
```bash
go test ./...
bin/riverpod plugins list --format json
bin/riverpod validate --config _examples/01-linear-etl.yaml --format json
