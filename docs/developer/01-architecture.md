...
| `internal/ir` | The static IR: DAG nodes/edges, compiled CEL/CESQL predicates and Starlark programs, topology checks, plugin + codec resolution, lint, job semantics. `ir.Build` *is* the build stage (with the retaining `BuildForExplain` for explain, released through `Pipeline.Close` — candidate 09) — reached only through `internal/verify` in production code | `engine`, `store`, anything runtime. IR is built before an engine exists |
...
| `internal/wasmhost` | wazero runtime host: wasip1 reactor compile (compiled modules cached by file identity + compile-affecting config, candidate 09), per-invoke budgets through the one `ResolveMode` resolution (fast mode default), guest ABI check, invokers | Nothing internal (leaf). `registry/builtin` adapts it |
...
| `internal/explain` | Deterministic walkthroughs: symbolic and message-level traces (resolved semantics — shared wasm mode, per-edge delivery + batch rule, optional drop, decoder-aware sample), mermaid/ASCII topology | `engine` — explain runs compiled IR, never a live engine; it dry-runs the retained `ForExplain` instances, and the caller closes them (`Pipeline.Close`, candidate 09) |
...
| `pkg/pluginproto` | Generated gRPC stubs for `eventboat.plugin.v1` | Generated code — regenerate, never hand-edit. It lives in `pkg/` (not `internal/`) so third-party plugin modules can import it |
| `pkg/plugin` | The compile-time plugin ABI: aliases of the engine's plugin interfaces plus typed `RegisterSource`/`RegisterTransform`/`RegisterSink`/`RegisterCodec` into the process-wide registry | Importing the engine — plugin packages import only this, so their own test binaries stay light |
| root `package eventboat` | `RunCLI` — the library entry point for custom builds: blank-import plugin packages, then run the full CLI under your own `main` (Benthos's `service.RunCLI` role) | Business logic — it delegates to `internal/cli` |
| `internal/cli` | The CLI verb table and cmdX executors (verify/test/run/trigger/jobs/explain/replay/repl/lsp/plugin/mcp), dispatch on lynx-go/commands, help goldens; shared by `cmd/eventboat` and the root package's `RunCLI` | New business logic — verbs delegate to `ops`, `testrun`, `explain`; and no `os.Args`/`os.Exit` — `Run` returns the exit code, the caller owns exit |
| `cmd/eventboat` | The shipped binary: a thin `main` over `internal/cli` | Anything else |
